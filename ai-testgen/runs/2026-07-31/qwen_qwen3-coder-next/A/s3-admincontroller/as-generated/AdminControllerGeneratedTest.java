...
class AdminControllerGeneratedTest {
    ...
    void setUp() {
        ...
        ReflectionUtils.setField(
            ReflectionUtils.findField(AdminController.class, "werkstattService"),
            adminController, werkstattService
        );
    }
    ...
    void admin_getMapping_returnsAdminView_withStatistikAndFirmaName() {
        ...
        verify(model, times(1)).addAttribute("statistikJson", any(String.class));
        verify(model, times(1)).addAttribute("firmaName", any(String.class));
        verify(model, times(1)).addAttribute("version", any(String.class));
        verify(model, times(1)).addAttribute("meldung", null);

        // Verify JSON is valid (basic check)
        ArgumentCaptor<String> jsonCaptor = ArgumentCaptor.forClass(String.class);
        verify(model).addAttribute("statistikJson", jsonCaptor.capture());
    ...
    void bereinigen_postMapping_returnsAdminView_withMeldung() {
        ...
        verify(model).addAttribute("meldung", meldungCaptor.capture());
