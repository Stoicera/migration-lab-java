...
class WerkstattServiceGeneratedTest {
    ...
    void loescheKunde_deletesKunde() {
        ...
        // Assert
        verify(jdbcTemplate).update(sqlCaptor.capture(), eq(1L));
    ...
    void getAlleFahrzeuge_returnsAllFahrzeugeWithKundeName() throws SQLException {
        ...
        when(rs.wasNull()).thenReturn(false);
        when(rs.getDate("pickerl_datum")).thenReturn(new Date(0));
        ...
        RowMapper<Fahrzeug> mapper = null;
        when(jdbcTemplate.query(anyString(), any(RowMapper.class)))
    ...
    void setzeStatus_allowsValidStatusTransitions() {
        ...
        // Act
        Auftrag result = service.setzeStatus(1L, "IN_ARBEIT");

        // Assert
        assertThat(result.getStatus()).isEqualTo("IN_ARBEIT");
    }
    ...
    void erstelleRechnung_createsRechnungWithCorrectAmounts() {
        ...
        when(jdbcTemplate.query(anyString(), any(RowMapper.class)))
                .thenReturn(List.of(auftrag))
                .thenReturn(Collections.emptyList()) // no existing rechnung
                .thenReturn(42) // max rechnung nr
                .thenReturn(123L); // new rechnung id

        // Act
        Rechnung result = service.erstelleRechnung(1L);

        // Assert
        assertThat(result.getRechnungNr()).isEqualTo("R-" + jahr + "-0043");
        assertThat(result.getSummeNetto()).isEqualTo(200.0);
        assertThat(result.getUst()).isEqualTo(40.0);
        assertThat(result.getSummeBrutto()).isEqualTo(240.0);
    }
    ...
    @Test
    void getMonatsBericht_returnsBerichtForYear() {
        // Arrange
        ...
        RowMapper<MonatsBericht> mapper = null;
        when(jdbcTemplate.query(anyString(), any(RowMapper.class)))
                .thenAnswer(invocation -> {
                    mapper = invocation.getArgument(1);
                    return List.of(mapper.mapRow(rs, 0));
    ...
    void bereinigeStornierte_deletesOldStornierteAuftraege() {
        ...
        verify(jdbcTemplate, times(2)).update(sqlCaptor.capture(), anyLong());
