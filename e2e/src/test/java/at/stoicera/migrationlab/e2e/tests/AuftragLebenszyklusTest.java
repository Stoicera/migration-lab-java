...
/**
 * Scenario: full repair-order lifecycle on a seed customer/vehicle — ANGENOMMEN → IN_ARBEIT (+
 * position) → FERTIG → ABGEHOLT.
 */
...
class AuftragLebenszyklusTest extends ScenarioTest {
  ...
  void auftragAnnehmen() {
    AuftragDetailPage detail =
        new AuftraegePage(driver, waits)
            .open()
            .neuerAuftrag()
            .kunde("HOFER, Franz")
            .fahrzeug("PE-123AB")
  ...
  void inArbeitNehmenUndPositionErfassen() {
    ...
    detail.inArbeit().addPosition("ARBEIT", "E2E Fehlersuche Bremse", "1.5", "78.00");
