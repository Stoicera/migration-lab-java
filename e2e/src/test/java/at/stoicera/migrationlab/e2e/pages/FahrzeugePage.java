...
/**
 * Global vehicle list (#/fahrzeuge). Read-only in the legacy UI — vehicles are
 * created and deleted on the owning customer's detail page (KundeDetailPage);
 * the view itself says so ("Fahrzeuge werden beim jeweiligen Kunden angelegt").
 */
public class FahrzeugePage {
	...
	/**
	 * Client-side filter (AngularJS filter:filter, no request). The digest has
	 * rendered when sendKeys returns; callers gate on waitForRowCount.
	 */
