...
/**
 * The selector maps are the ONLY thing that differs between the two UIs — so their key sets must be
 * identical, always. Stage 1–4 kept this by copy; stage 5 edits modern.properties key by key while
 * the Angular UI grows, and this guard turns a forgotten/mistyped key into a red build instead of a
 * "selector key missing" failure halfway through a scenario (or worse, a green run that silently
 * tested the wrong UI). Target-independent by construction: it loads both files directly, not via
 * {@link SelectorMap}.
 */
