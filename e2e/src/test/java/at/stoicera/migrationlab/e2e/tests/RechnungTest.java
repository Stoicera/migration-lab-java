...
/**
 * Scenario: invoice creation from the seed order A-2026-0009 (FERTIG, Klimaservice, 123.00 netto),
 * detail navigation from the list, and the server-side rejection of a second invoice for the same
 * order. The number of the created invoice depends on the wall-clock year — computed in {@link
 * Seed#naechsteRechnungNr()}, never hard-coded.
 */
@DisplayName(
    "Rechnung: erstellen aus fertigem Auftrag, 20% USt, bezahlt setzen, Detail, Doppel-Abweisung")
class RechnungTest extends ScenarioTest {
  ...
  void rechnungDetailAusDerListe() {
    ...
    RechnungDetailPage detail =
        new RechnungenPage(driver, waits).open().openRechnung("R-2026-0004");
    ...
    assertThat(positionen.get(0))
        .containsExactly("Zahnriemen + WaPu tauschen", "4.5", "88,00 €", "396,00 €");
