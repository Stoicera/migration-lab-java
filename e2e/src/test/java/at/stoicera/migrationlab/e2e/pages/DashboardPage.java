...
public class DashboardPage {
	...
	/**
	 * The two dashboard tables have no ids in the 2016 markup — the selectors are
	 * positional (column div order). Tests MUST pin the section headings below so
	 * a silent reordering fails loudly instead of asserting the wrong table.
	 */
	...
	private List<List<String>> zeilen(String rowsKey) {
		return driver.findElements(css(rowsKey)).stream()
				.map(row -> row.findElements(css("dashboard.rowCells")).stream()
