...
/**
 * Scenario: vehicle module. The global list is read-only in the legacy UI; create/delete happen on
 * the owning customer's detail page — the only real flow (the standalone list even says so). Uses a
 * Kennzeichen that never occurs in the seed, so list assertions are unambiguous.
 */
...
class FahrzeugeTest extends ScenarioTest {
  ...
  void listeZeigtSeedFahrzeuge() {
    ...
    assertThat(liste.zeile("PE-123AB"))
        .containsExactly(
  ...
  void fahrzeugBeimKundenAnlegen() {
    ...
    assertThat(liste.zeile(KENNZEICHEN))
        .containsExactly(KENNZEICHEN, "Dacia", "Duster", "2019", "55000", "", "Hofer Franz");
  }
