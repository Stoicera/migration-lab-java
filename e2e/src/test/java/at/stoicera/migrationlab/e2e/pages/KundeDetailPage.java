...
public class KundeDetailPage {
  ...
  /**
   * Data-load gate for EXISTING customers (flaky log race: kunde-detail.html has no ng-if around
   * the form — the empty template renders before the async GET populates the model). Reading before
   * this gate returns empty strings; typing before it is worse: the late response overwrites the
   * model and silently discards the input. Every navigation to an existing customer must pass
   * through here (KundenPage.openByName does). The new-customer form (KundenPage.neuerKunde) is
   * legitimately empty and must NOT use this gate.
   */
  public KundeDetailPage waitLoaded() {
    // both GETs (kunde + fahrzeuge) done -> model applied, vehicle table rendered
    waits.idle();
    waits.until(
        css("kundeDetail.nachname"),
        feld -> {
  ...
  /**
   * Saves and waits for the round trip to COMPLETE. The heading check alone is vacuous for edits
   * (it already shows the name before saving) and the legacy UI gives no save feedback — without
   * the idle wait, navigating on can abort the in-flight PUT and lose the update (flaky log #3,
   * found in CI).
   */
  ...
  /**
   * Saves expecting the client-side validation alert (e.g. missing Nachname); returns the alert
   * text so the test pins the exact message. No idle wait: the validation fires before any request.
   */
  ...
  public List<List<String>> fahrzeugZeilen() {
    return driver.findElements(css("kundeDetail.fahrzeugRows")).stream()
        .map(
            row ->
                row.findElements(css("kundeDetail.fahrzeugRowCells")).stream()
                    .map(WebElement::getText)
                    .toList())
        .toList();
  }

  public KundeDetailPage fahrzeugHinzufuegen(
      String kennzeichen, String marke, String modell, String baujahr, String kmStand) {
