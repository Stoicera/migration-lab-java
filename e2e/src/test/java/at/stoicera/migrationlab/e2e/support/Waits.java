...
public final class Waits {
   ...
   * the UI under test, not of the scenario: today both stands serve the AngularJS UI ({@code
   * angularjs}); the stage-5 Angular UI gets an {@code angular} strategy against the Angular
   * testability API. Unknown values throw — fail-loud by design, a silently skipped idle wait would
   * reintroduce the lost-update race of flaky log #3.
   */
   ...
   * AngularJS' $http.pendingRequests. The stage-5 Angular UI adds an "angular" strategy against the
   * Angular testability API — this method throws on anything unknown instead of guessing, because a
   * wrong strategy times out on every save (angular UI) or silently skips the gate.
   */
  public void idle() {
   ...
   * $http.pendingRequests (see modern/frontend/src/app/offene-requests.interceptor.ts). Strictly
   * typed probe: a missing marker is "not idle", never "skip the gate".
   */
   ...
   * carries. Neither marker present = not idle (e.g. mid page-swap) — the wait polls on, fail-loud
   * on timeout.
   */
