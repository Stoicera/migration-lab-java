 ...
 *
 * Year handling: the tests select the SEED year 2026 explicitly. That stays
 * possible on any wall-clock date because the legacy year dropdown is built
 * from the current year down to 2016. While the wall clock is in 2026 the
 * selection is a no-op (already selected); from 2027 on it becomes a real
 * change — both paths are deterministic, see BerichtPage.jahr.
 */
...
class BerichtTest extends ScenarioTest {
	...
	@Order(1)
	void jaennerWerte() {
		BerichtPage bericht = new BerichtPage(driver, waits).open()
	...
	void jahresWechselLaedtNeu() {
		...
		assertThat(bericht.monthRow("Jänner"))
				.containsExactly("Jänner", "0", "0", "0,00 €", "0,00 €");
