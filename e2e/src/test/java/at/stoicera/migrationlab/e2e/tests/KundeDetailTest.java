...
/**
 * Scenario: opening a seed customer shows the full master data and the
 * customer's vehicles. (The legacy kunde-detail view shows NO orders — only
 * master data and vehicles; order history lives in the Aufträge list.)
 * Exercises the data-load gate: the form template renders empty before the
...
class KundeDetailTest extends ScenarioTest {
	...
	void stammdatenUndFahrzeugeAusDemSeed() {
		...
		assertThat(fahrzeuge.get(0)).containsExactly("PE-123AB", "VW Golf V 1.9 TDI", "2006", "30.11.2026", "X");
		assertThat(fahrzeuge.get(1)).containsExactly("PE-456CD", "Skoda Octavia Combi", "2015", "31.03.2027", "X");
	}
}
