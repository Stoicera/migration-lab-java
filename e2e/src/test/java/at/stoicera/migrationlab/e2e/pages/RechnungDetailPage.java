...
public class RechnungDetailPage {
  ...
  /**
   * Cell texts of all position rows: [Position, Menge, Einzelpreis, Betrag]. The view loads the
   * invoice first, then the order NESTED (second GET fires only after the first resolves) — the row
   * wait gates on the second load; all rows of one order render in a single digest.
   */
  public List<List<String>> positionZeilen() {
    ...
    return driver.findElements(css("rechnungDetail.posRows")).stream()
        .map(
            row ->
                row.findElements(css("rechnungDetail.rowCells")).stream()
                    .map(WebElement::getText)
                    .toList())
        .toList();
  }
