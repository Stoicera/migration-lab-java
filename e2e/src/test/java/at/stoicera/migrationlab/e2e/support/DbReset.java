...
public final class DbReset {
	...
	/**
	 * Line-based splitter: a statement ends where a line ends with ';'. That is
	 * enough for the canonical seed and deliberately NOT a SQL parser — but it
	 * would silently corrupt statements containing ';' inside string literals or
	 * dollar-quoted blocks. The guards below turn that silent corruption into a
	 * loud failure pointing here, so whoever extends the seed finds out at the
	 * first run, not via a mysteriously half-loaded database.
	 */
	private static List<String> parseStatements(Path seedFile) throws Exception {
		...
		for (String line : Files.readAllLines(seedFile)) {
			...
			if (line.contains("$$")) {
				throw new IllegalStateException("Seed file " + seedFile + " contains a dollar-quote ($$). "
		...
		if (!current.toString().isBlank()) {
			throw new IllegalStateException("Seed file " + seedFile + " ends with an unterminated statement "
	...
	/**
	 * An odd number of single quotes means the split cut through a string
	 * literal (e.g. a ';' inside a literal at a line end). SQL escapes quotes by
	 * doubling them (''), which keeps the count even — so odd is always a split
	 * error, never legal seed content.
	 */
	private static void requireBalancedQuotes(String statement, Path seedFile) {
		long quotes = statement.chars().filter(c -> c == '\'').count();
		if (quotes % 2 != 0) {
			throw new IllegalStateException("Statement with unbalanced single quotes after splitting "
					+ seedFile + " — most likely a ';' inside a string literal at a line end, which the "
