...
public class AuftragNeuPage {
  ...
  /**
   * Customer options load async (GET api/kunden fired on controller start) — same guard as {@link
   * #fahrzeug(String)}: wait for the wanted entry before selecting, selectByVisibleText on a
   * not-yet-populated select throws.
   */
  public AuftragNeuPage kunde(String anzeigeName) {
    waits.until(
        css("auftragNeu.kundeSelect"),
        select ->
            new Select(select)
                .getOptions().stream().anyMatch(o -> o.getText().equals(anzeigeName)));
  ...
  /**
   * Submits expecting the client-side validation alert (customer/vehicle missing); returns the
   * alert text so the test pins the exact message.
   */
