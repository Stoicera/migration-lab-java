...
public class BerichtPage {
  ...
  /**
   * Selects a report year and waits until the table PROVABLY shows that year's data. Two cases,
   * both deterministic:
   *
   * <p>1. The requested year is already selected (initial controller state is the wall-clock year,
   * so this is the everyday case while that is 2026): selecting the selected option fires NO change
   * event, ng-change="laden()" does not run — a deliberate no-op. The visible data already belongs
   * to that year (loaded by the controller's initial laden()).
   *
   * <p>2. A real change: ng-change fires two GETs, but waiting on the month-row count is vacuous —
   * the API always returns 12 rows (generate_series) and ng-repeat re-renders them in place. The
   * only honest gate is a cell whose value provably differs between the two years. Callers pass the
   * expected "Gesamt / Aufträge" cell ({@code #summe-auftraege}): on a fresh seed it is 16 for 2026
   * and 0 for every other year, so any year switch the suite performs crosses a visible value
   * change. The exact-text wait then also covers case 1 for free (the cell already carries the
   * value).
   */
  ...
  public List<String> topKundeErsteZeile() {
    ...
    return first.findElements(css("bericht.rowCells")).stream().map(WebElement::getText).toList();
  }
}
