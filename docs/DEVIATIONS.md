...
| Lint/Format-Check in CI | waived for `legacy/`, deferred(G5) for the rest | A formatter on `legacy/` would destroy the 2016 exhibit (hard rule: legacy stays legacy). For `modern/`+test modules a format check is added at G5 when the Angular toolchain (with its own lint) arrives anyway — one formatting story, once. |
