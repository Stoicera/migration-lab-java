 ...
 * property.
 *
 * <p>On failure the received document is written to target/characterization-received/ for diffing
 * against src/test/resources/golden/.
 */
...
class ApiCharacterizationTest {
  ...
  void sucheMitEinzelnemHochkomma() throws Exception {
    ...
    if (Stand.isModern()) {
      ...
      assertThat(body.path("exception").asText())
          .isEqualTo("java.lang.ArrayIndexOutOfBoundsException");
  ...
  private JsonNode readGolden(String name) throws Exception {
    try (InputStream in = getClass().getResourceAsStream("/golden/" + name)) {
      assertThat(in)
          .as("golden file missing: " + name + " — capture it per characterization/README.md")
          .isNotNull();
