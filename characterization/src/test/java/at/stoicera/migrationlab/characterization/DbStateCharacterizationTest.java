...
/**
 * Characterizes the DB state transitions of the write paths — the complete
 * status transition matrix, number generation, invoice amounts (including a
 * double-rounding boundary case) and the quirks (vehicle km side effect,
 * orphaned rows on delete, kept fertig_am on the FERTIG→IN_ARBEIT special
 * case). These document CURRENT behaviour; the migration must reproduce it
 * until a stage explicitly changes it (with ADR + playbook entry).
 *
 * Every test resets the DB to the committed seed first and works only against
 * seed rows — no ordering dependency between tests. Seed anchors used below:
 * Auftrag 1 ABGEHOLT, 9/12 FERTIG, 11/13 IN_ARBEIT, 14/15 ANGENOMMEN,
 * 4/16 STORNIERT; Rechnung 5 unbezahlt.
 */
...
class DbStateCharacterizationTest {
	...
	void angenommenNachInArbeit_setztNurStatus() throws Exception {
		...
		try (Connection c = Stand.connect(); Statement s = c.createStatement()) {
			ResultSet rs = s.executeQuery("SELECT status, fertig_am, abgeholt_am FROM auftrag WHERE id = 15");
	...
	void angenommenNachStorniert_setztNurStatus() throws Exception {
		...
		try (Connection c = Stand.connect(); Statement s = c.createStatement()) {
			ResultSet rs = s.executeQuery("SELECT status, fertig_am, abgeholt_am FROM auftrag WHERE id = 14");
			assertThat(rs.next()).isTrue();
	...
	void inArbeitNachFertig_setztZeitstempel() throws Exception {
		...
		try (Connection c = Stand.connect(); Statement s = c.createStatement()) {
			ResultSet rs = s.executeQuery("SELECT status, fertig_am, abgeholt_am FROM auftrag WHERE id = 11");
			assertThat(rs.next()).isTrue();
	...
	void inArbeitNachStorniert_setztNurStatus() throws Exception {
		...
		try (Connection c = Stand.connect(); Statement s = c.createStatement()) {
			ResultSet rs = s.executeQuery("SELECT status, fertig_am, abgeholt_am FROM auftrag WHERE id = 13");
			assertThat(rs.next()).isTrue();
			assertThat(rs.getString("status")).isEqualTo("STORNIERT");
			assertThat(rs.getTimestamp("fertig_am")).isNull();
			assertThat(rs.getTimestamp("abgeholt_am")).isNull();
	...
	void fertigZurueckInArbeit_sonderfallBehaeltFertigAm() throws Exception {
		...
		try (Connection c = Stand.connect(); Statement s = c.createStatement()) {
			ResultSet rs = s.executeQuery("SELECT status, fertig_am, abgeholt_am FROM auftrag WHERE id = 9");
			assertThat(rs.next()).isTrue();
			assertThat(rs.getString("status")).isEqualTo("IN_ARBEIT");
			assertThat(rs.getTimestamp("fertig_am"))
					.isEqualTo(Timestamp.valueOf("2026-05-20 09:30:00"));
			assertThat(rs.getTimestamp("abgeholt_am")).isNull();
		}
	}
	...
	void fertigNachAbgeholt_setztAbgeholtAmUndBehaeltFertigAm() throws Exception {
		...
		try (Connection c = Stand.connect(); Statement s = c.createStatement()) {
			ResultSet rs = s.executeQuery("SELECT status, fertig_am, abgeholt_am FROM auftrag WHERE id = 12");
			assertThat(rs.next()).isTrue();
			assertThat(rs.getString("status")).isEqualTo("ABGEHOLT");
			assertThat(rs.getTimestamp("fertig_am"))
					.isEqualTo(Timestamp.valueOf("2026-07-08 13:10:00"));
	...
	void rechnungRundung_doppeltesRundenAmGrenzfall() throws Exception {
		...
		var auftrag = Stand.send("POST", "/api/auftraege",
	...
	void adminBereinigen_loeschtGenauStornierteAelterAls90Tage() throws Exception {
		...
		try (Connection c = Stand.connect(); Statement s = c.createStatement()) {
			ResultSet rs = s.executeQuery("SELECT id FROM auftrag WHERE status = 'STORNIERT' "
			...
			for (Long id : erwartetGeloescht) {
				ResultSet pn = s.executeQuery(
						"SELECT COUNT(*) AS n FROM auftrag_position WHERE auftrag_id = " + id);
				pn.next();
		...
		try (Connection c = Stand.connect(); Statement s = c.createStatement()) {
			for (Long id : erwartetGeloescht) {
				...
				ResultSet positionen = s.executeQuery(
						"SELECT COUNT(*) AS n FROM auftrag_position WHERE auftrag_id = " + id);
				positionen.next();
				assertThat(positionen.getInt("n")).as("Positionen zu Auftrag " + id + " geloescht").isZero();
			}
			ResultSet auftraege = s.executeQuery("SELECT COUNT(*) AS n FROM auftrag");
			auftraege.next();
			assertThat(auftraege.getInt("n")).as("alle anderen Auftraege ueberleben")
			...
			positionen.next();
			assertThat(positionen.getInt("n")).as("alle anderen Positionen ueberleben")
