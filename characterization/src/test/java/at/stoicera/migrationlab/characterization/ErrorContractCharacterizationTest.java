...
/**
 * Pins the 4xx/5xx surface the UIs actually consume: 404-without-body for
 * missing resources, the 500-with-German-message contract of the write paths,
 * and the Boot default error JSON where no controller catch exists
 * (LEGACY_NOTES B12: no input validation — the DB NOT-NULL constraint is the
 * only "validation" and its failure reaches the client).
 *
 * Legacy and modern must answer identically on every path here. The modern
 * stand needs a wire-compat shim for the two error-JSON fields ("exception",
 * "message") that Boot 2+ hides by default — see spring.web.error.* in
 * modern/src/main/resources/application.properties (same philosophy as
 * JacksonWireCompatConfig, ADR-0005).
 *
 * Every test resets the DB to the committed seed first — no ordering
 * dependency, safe for the one test that mutates state (doppelteRechnung).
 */
...
class ErrorContractCharacterizationTest {
	...
	@ValueSource(strings = { "/api/kunden/999", "/api/auftraege/999", "/api/rechnungen/999", "/api/fahrzeuge/999" })
